...
def generate_grassmann_indices(N_trunc):
    ...
    for i in range(N_trunc):
        generators_epsilon_conj.append('conj'+str(i))       
    c2 = gtn.set_ac(generators_epsilon_conj)
    eye_aux=np.eye(N_trunc,dtype=complex )
    eye_aux = gtn.dense(data=eye_aux, statistics=(-1,-1))

    dc1 = gtn.arith.d(c1)
    dc2 = gtn.arith.d(c2)
    dc3 = dc1*dc2
    berezin_measures={'c1':dc1,'c2':dc2,'c3':dc3 }

    return c1, c2, c1*c2,eye_aux,berezin_measures
